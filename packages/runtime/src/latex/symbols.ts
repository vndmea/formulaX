...
export type RuntimeResolvedSymbol = {
  char: string;
  fontFamily?: string;
  latex: string;
};
...
export const SYMBOL_TO_LATEX = Object.entries(LATEX_SYMBOLS).reduce<Record<string, string>>(
  (result, [command, symbol]) => {
    if (command.length === 1 && command === symbol) {
      return result;
    }
    if (!(symbol in result)) {
...
export function resolveRuntimeSymbol(key: string): RuntimeResolvedSymbol | null {
  ...
  const code = LATEX_SYMBOLS[clean];
  if (code !== undefined) {
    return {
      char: code,
      latex: key,
