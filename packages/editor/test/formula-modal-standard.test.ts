...
describe('mountFormulaXEditor runtime=standard', () => {
  it('allows runtime standard to be selected as the editor default', async () => {
    ...
    setDefaultFormulaXEditorRuntime('standard');

    try {
      expect(getDefaultFormulaXEditorRuntime()).toBe('standard');
  ...
  it('auto-selects runtime standard when standard runtime options are provided', async () => {
    document.body.innerHTML = '<div id="host"></div>';
    const host = document.getElementById('host') as HTMLElement;
    const mounted = mountFormulaXEditor(host, {
      runtime: 'auto',
  ...
  it('renders toolbar previews through renderer-standard without mounting history controls', async () => {
    document.body.innerHTML = '<div id="host"></div>';
    const host = document.getElementById('host') as HTMLElement;
    const mounted = mountFormulaXEditor(host, {
      runtime: 'standard',
  ...
  it('does not emit kf or kity class names in the standard DOM', async () => {
    document.body.innerHTML = '<div id="host"></div>';
    const host = document.getElementById('host') as HTMLElement;
    const mounted = mountFormulaXEditor(host, {
      runtime: 'standard',
