export class IntegrationOperatorModule {
  static create(kity: any, Operator: any, ScriptController: any) {
    return kity.createClass("IntegrationOperator", {
      ...
      applyOperand: function (exp: any, sup: any, sub: any) {
        const opShape = this.getOperatorShape();
        const padding = 3;
        const expBox = exp.getFixRenderBox();
        const space = new ScriptController(this, opShape, sup, sub, {
          supOffset: 3,
          subOffset: -15
        }).applySide();
        let diff = (space.height + space.top - expBox.height) / 2;
...
export const createIntegrationOperatorClass = IntegrationOperatorModule.create;
