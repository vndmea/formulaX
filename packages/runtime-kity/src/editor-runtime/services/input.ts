...
const InputComponent = kity.createClass('InputComponent', {
  constructor(this: InputComponentInstance, parentComponent: unknown, kfEditor: KityRuntimeEditorInstance) {
  ...
  initEvent(this: InputComponentInstance) {
    kityBaseUtils.addEvent(this.inputBox, 'keydown', (e: KeyboardEvent) => {
      let isControl = false;
    ...
    kityBaseUtils.addEvent(this.inputBox, 'input', () => {
      this.processingInput();
    });

    kityBaseUtils.addEvent(this.inputBox, 'blur', () => {
    ...
    kityBaseUtils.addEvent(this.inputBox, 'focus', () => {
    ...
    kityBaseUtils.addEvent(this.inputBox, 'paste', (e: ClipboardEvent) => {
      e.preventDefault();
    });
  },
  ...
  pretreatmentInput(this: InputComponentInstance, evt: KeyboardEvent) {
    ...
    const replaceStr = kityInputFilter.getReplaceString(keyCode);
...
export default InputComponent as new (parentComponent: unknown, kfEditor: KityRuntimeEditorInstance) => InputComponentInstance;
