...
type KityRectShape = {
  fill: (color: string) => KityRectShape;
  setAttr: (name: string, value: string) => void;
  setHeight: (height: number) => void;
  getTransform: (shape: KityRectShape) => {
    m: {
      e: number;
      f: number;
    };
...
type KityWithRect = ReturnType<typeof getKityGraphicsRuntime> & {
  Rect: new (width: number, height: number, x: number, y: number) => KityRectShape;
};
...
type PaperLike = {
  ...
  addShape: (shape: KityRectShape) => void;
  getZoom: () => number;
};
...
type LocationComponentInstance = {
  ...
  paper: PaperLike;
  cursorShape: KityRectShape;
  getPaper: () => PaperLike;
  initServices: () => void;
  initEvent: () => void;
  createCursor: () => KityRectShape;
...
const kity = getKityGraphicsRuntime() as KityWithRect;

const LocationComponent = kity.createClass('LocationComponent', {
  constructor(this: LocationComponentInstance, parentComponent: unknown, kfEditor: KityRuntimeEditorInstance) {
  ...
  updateCursor(this: LocationComponentInstance) {
    ...
    const paperContainerRect = kityBaseUtils.getRect(this.paper.container.node);
    const focusChildRect = kityBaseUtils.getRect(focusChild);
  ...
  getIndex(this: LocationComponentInstance, distance: number, groupInfo: GroupInfo) {
    ...
    for (let i = groupInfo.content.length - 1; i >= 0; i -= 1) {
      ...
      const boundingRect = kityBaseUtils.getRect(child);
...
export default LocationComponent as new (parentComponent: unknown, kfEditor: KityRuntimeEditorInstance) => LocationComponentInstance;
