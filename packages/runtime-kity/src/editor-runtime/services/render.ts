...
type KityContainerTranslate = {
  x: number;
  y: number;
};

type KityFormulaContainer = {
  getRenderBox: (scope?: string) => KityRenderBox;
  setTranslate: (x: number | KityContainerTranslate, y?: number) => void;
  getTranslate: () => KityContainerTranslate;
};

type KityFormula = {
  container: KityFormulaContainer;
  node: {
    removeAttribute: (name: string) => void;
  };
  getViewBox: () => KityViewBox;
  setViewBox: (x: number, y: number, width: number, height: number) => void;
  getViewPort: () => KityViewport;
  setViewPort: (viewPort: KityViewport) => void;
  getContentContainer: () => {
    getRenderBox: (scope?: string) => KityRenderBox;
  };
};
...
type KityFormulaAssemblyRuntime = ReturnType<typeof getKityFormulaRuntime> & {
  Assembly: new (formula: unknown) => KityAssembly;
};
...
type SelectableGroupObject = {
  ...
  operands: Array<{
    getRenderBox: (group: SelectableGroupObject) => KityRenderBox;
    node: Element;
  }>;
...
type RenderComponentRecord = {
  ...
  canvas: {
    viewBox?: KityViewBox | null;
    contentOffset?: KityContainerTranslate | null;
  };
};
...
type RenderComponentInstance = {
  ...
  reselect: () => void;
  clearSelect: () => void;
  getPaper: () => KityFormula;
  getPaperOffset: () => KityContainerTranslate;
  render: (latexStr: string) => void;
  enableRelocation: () => void;
  disableRelocation: () => void;
  setCanvasZoom: (zoom: number) => void;
  getCanvas: () => KityFormula;
  getContentSize: () => KityRenderBox;
...
const kity = getKityGraphicsRuntime() as ReturnType<typeof getKityGraphicsRuntime> & {
  Utils: {
    extend: <T>(deep: boolean | T, target?: T, ...sources: Array<Record<string, unknown>>) => T;
  };
};
const kf = getKityFormulaRuntime() as KityFormulaAssemblyRuntime;
...
const RenderComponent = kity.createClass('RenderComponent', {
  ...
  constructor(this: RenderComponentInstance, kfEditor: KityRuntimeEditorInstance, options?: Record<string, unknown>) {
    ...
    this.assembly = null;
    this.formula = null as unknown as KityFormula;
  ...
  initCanvas(this: RenderComponentInstance) {
    ...
    const Formula = this.kfEditor.getFormulaClass() as new (container: HTMLElement, options: unknown) => KityFormula;

    this.assembly = new kf.Assembly(new Formula(canvasContainer, this.options)) as KityAssembly;
...
export default RenderComponent as new (kfEditor: KityRuntimeEditorInstance, options?: Record<string, unknown>) => RenderComponentInstance;
