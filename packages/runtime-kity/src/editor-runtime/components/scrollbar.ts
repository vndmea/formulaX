...
const SCROLLBAR_DEF = kityUiConfig.scrollbar;
const SCROLLBAR_CONF = kitySystemConfig.scrollbar;
...
function preventDefault(comp: ScrollbarInstance) {
  kityBaseUtils.addEvent(comp.container, 'mousedown', (e: Event) => {
    e.preventDefault();
  });
}

function trackClick(comp: ScrollbarInstance) {
  kityBaseUtils.addEvent(comp.widgets!.track, 'mousedown', function (this: HTMLDivElement, e: MouseEvent) {
    trackClickHandler(this, comp, e);
  });
}

function btnClick(comp: ScrollbarInstance) {
  kityBaseUtils.addEvent(comp.widgets!.leftButton, 'mousedown', () => {
    setThumbOffsetByStep(comp, -SCROLLBAR_CONF.step);
  });

  kityBaseUtils.addEvent(comp.widgets!.rightButton, 'mousedown', () => {
    setThumbOffsetByStep(comp, SCROLLBAR_CONF.step);
  });
}
...
function thumbHandler(comp: ScrollbarInstance) {
  ...
  kityBaseUtils.addEvent(comp.widgets!.thumb, 'mousedown', (e: MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  kityBaseUtils.addEvent(comp.container.ownerDocument, 'mouseup', () => {
    isMoving = false;
    startPoint = 0;
    startOffset = 0;
  });

  kityBaseUtils.addEvent(comp.container.ownerDocument, 'mousemove', (e: MouseEvent) => {
    if (!isMoving) {
      return;
    }
...
function trackClickHandler(track: HTMLDivElement, comp: ScrollbarInstance, evt: MouseEvent) {
  const trackRect = kityBaseUtils.getRect(track);
...
const Scrollbar = kity.createClass('Scrollbar', {
  constructor(this: ScrollbarInstance, uiComponent: ScrollbarUiComponent, kfEditor: KityRuntimeEditorInstance) {
  ...
  initSize(this: ScrollbarInstance) {
    const leftBtnWidth = kityBaseUtils.getRect(this.widgets!.leftButton).width;
    const rightBtnWidth = kityBaseUtils.getRect(this.widgets!.rightButton).width;

    this.values.viewWidth = kityBaseUtils.getRect(this.container).width;
...
export default Scrollbar as new (uiComponent: ScrollbarUiComponent, kfEditor: KityRuntimeEditorInstance) => ScrollbarInstance;
