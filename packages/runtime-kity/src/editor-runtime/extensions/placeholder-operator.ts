...
const kity = getKityGraphicsRuntime() as any;
...
function createRootPlaceholder(operator: PlaceholderOperatorInstance, label: string) {
  const textShape = new kity.Text(label).fill(kitySystemConfig.rootPlaceholder.color);
  const shapeGroup = new kity.Group();
  const padding = 20;
  const radius = 7;
  const borderBoxShape = new kity.Rect(0, 0, 0, 0, radius).stroke(kitySystemConfig.rootPlaceholder.color).fill('transparent');
  ...
  borderBoxShape.stroke(kitySystemConfig.rootPlaceholder.color).fill('transparent');
...
const PlaceholderOperator = kity.createClass('PlaceholderOperator', {
  ...
  select(this: PlaceholderOperatorInstance) {
    this.opShape?.fill(kityExtensionStyle.selectColor);
  },

  selectAll(this: PlaceholderOperatorInstance) {
    this.opShape?.fill(kityExtensionStyle.allSelectColor);
  },
