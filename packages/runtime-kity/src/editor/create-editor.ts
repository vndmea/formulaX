import { kityAssetManifest, type KityAssetManifest } from '../assets/manifest';
import { kityBoxType } from '../kity-compat/box-type';
import { kityCharPosition } from '../kity-compat/char-position';
import { kityOtherPosition } from '../kity-compat/other-position';
import { kityCommonUtils } from '../kity-compat/common-utils';
import { createKityBaseComponent } from '../kity-compat/base-component';
import { kityElementType } from '../kity-compat/element-type';
import { kityEventListener } from '../kity-compat/event-listener';
import { kityGroupType } from '../kity-compat/group-type';
import { kityInputFilter } from '../kity-compat/input-filter';
import { kityItemType } from '../kity-compat/item-type';
import { kityExtensionStyle } from '../kity-compat/extension-style';
import { kityEvent } from '../kity-compat/kf-event';
import { kitySystemConfig } from '../kity-compat/system-config';
import { kityUiConfig } from '../kity-compat/ui-config';
import { kityUiUtils } from '../kity-compat/ui-utils';
...
import { setToolbarAssetUrls } from '../assets/toolbar';
import {
...
type KityWindow = Window &
  typeof globalThis & {
    ...
    __FORMULAX_KITY_RUNTIME__?: {
      ...
      baseUtils?: typeof kityBaseUtils;
      boxType?: typeof kityBoxType;
      sysconf: typeof kitySystemConfig;
      charPosition: typeof kityCharPosition;
      commonUtils?: typeof kityCommonUtils;
      eleType?: typeof kityElementType;
      eventListener?: typeof kityEventListener;
      groupType?: typeof kityGroupType;
      inputFilter?: typeof kityInputFilter;
      itemType?: typeof kityItemType;
      kf?: unknown;
      kfExtDef?: typeof kityExtensionStyle;
      kfEvent?: typeof kityEvent;
      kity?: unknown;
      otherPosition: typeof kityOtherPosition;
      uiDef?: typeof kityUiConfig;
      uiUtils: typeof kityUiUtils;
    };
...
function hydrateKityFormulaRuntime(runtimeWindow: KityWindow) {
...
function installKityCompatRuntime(runtimeWindow: KityWindow) {
  runtimeWindow.__FORMULAX_KITY_RUNTIME__ = {
    baseComponent: runtimeWindow.kity
      ? createKityBaseComponent(
          runtimeWindow.kity as { createClass: (name: string, definition: object) => unknown },
        )
      : undefined,
    baseUtils: kityBaseUtils,
    boxType: kityBoxType,
    sysconf: kitySystemConfig,
    charPosition: kityCharPosition,
    commonUtils: kityCommonUtils,
    eleType: kityElementType,
    eventListener: kityEventListener,
    groupType: kityGroupType,
    inputFilter: kityInputFilter,
    itemType: kityItemType,
    kf: runtimeWindow.kf,
    kfExtDef: kityExtensionStyle,
    kfEvent: kityEvent,
...
export async function ensureKityRuntime() {
  ...
  runtimePromise = (async () => {
    ...
    try {
      ...
      const { default: installKityFormulaRuntime } = await import('../vendor/kity-formula/install');
      ...
      installKityFormulaRuntime(runtimeWindow);
      hydrateKityFormulaRuntime(runtimeWindow);
      ...
      const { default: installKityParserRuntime } = await import('../vendor/kity-formula-parser/install');
      ...
      installKityParserRuntime(runtimeWindow);
      ...
      installKityCompatRuntime(runtimeWindow);

      const bootImportStart = markFormulaXPerf('fx:kity-runtime:boot-import');
      const { installKityEditorStart } = await import('./boot');
...
export async function createKityEditor(
  ...
  try {
    kitySystemConfig.rootPlaceholder.content = getFormulaXRuntimeMessage('editor.placeholder.root', locale);
