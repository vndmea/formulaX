...
function applyKityMixins(NewClass: KityCtor, mixins: KityCtor[]): KityCtor {
...
function checkBaseConstructorCall(targetClass: AnyFunc, classname: string) {
  if (!/this\.callBase/.test(targetClass.toString())) {
    throw new Error(
      `${classname}: the constructor must call the base constructor for compatibility with the Kity class system.`,
    );
  }
}
...
export function createClass<T = Record<string, any>>(
  ...
  NewClass = applyKityMixins(NewClass, defines.mixins || []);
